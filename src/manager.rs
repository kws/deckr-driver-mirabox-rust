...
struct MiraBoxHardwareHandler {
    command_map: Mutex<HashMap<String, CommandSender>>,
}

impl MiraBoxHardwareHandler {
...
impl HardwareCommandHandler for MiraBoxHardwareHandler {
...
impl HardwareResetHandler for MiraBoxHardwareHandler {
...
impl MiraBoxRemoteManager {
    ...
    async fn run_connected_session(&self) -> Result<()> {
        ...
        let hardware_handler = Arc::new(MiraBoxHardwareHandler::default());
        ...
        .await
        .context("starting managed MiraBox hardware runtime")?;
        ...
        hardware_runtime
            ...
            .await
            .context("starting managed MiraBox hardware runtime tasks")?;
        ...
        tokio::select! {
            ...
            result = hardware_tasks.join_next() => {
                ...
                let _ = hardware_runtime.stop().await;
                match result {
                    Some(Ok(Ok(()))) => bail!("managed MiraBox hardware runtime task stopped unexpectedly"),
                    Some(Ok(Err(error))) => Err(error).context("managed MiraBox hardware runtime task"),
                    Some(Err(error)) => Err(error).context("joining managed MiraBox hardware runtime task"),
                    None => bail!("managed MiraBox hardware runtime tasks stopped unexpectedly"),
                }
            }
        }
    }
...
async fn worker_event_loop(
    ...
    hardware_handler: Arc<MiraBoxHardwareHandler>,
    ...
    while let Some(event) = worker_rx.recv().await {
        match event {
            ...
            WorkerEvent::Input { device_id, body } => {
                debug!("Routing MiraBox hardware input for {device_id}");
...
mod tests {
    ...
    struct ManagedHarness {
        ...
        handler: Arc<MiraBoxHardwareHandler>,
    }
    ...
    async fn managed_harness() -> ManagedHarness {
        ...
        let handler = Arc::new(MiraBoxHardwareHandler::default());
    ...
    async fn managed_runtime_accepts_claim_and_delivers_command_without_duplicate_token() {
        ...
        let (command_tx, command_rx) = command_channel();
        h.handler
            .register_device("deck".to_string(), command_tx)
            .await;
        h.runtime
            .set_device(test_device_descriptor("deck", "fingerprint:deck"))
            .await
            .unwrap();
        let contract = create_claim(&h.concord, "claim-a", "deck", Some("fingerprint:deck")).await;
    ...
    async fn managed_runtime_rejects_wrong_controller_and_unsupported_command() {
        ...
        h.runtime
            ...
            .await
            .unwrap();
        create_claim(&h.concord, "claim-a", "deck", Some("fingerprint:deck")).await;
    ...
    async fn managed_runtime_resets_on_lost_claim_and_cancels_on_disconnect() {
        ...
        h.runtime
            ...
            .await
            .unwrap();
        let first = create_claim(&h.concord, "claim-a", "deck", Some("fingerprint:deck")).await;
        h.lane.publish_inbound(
            DeckrMessage::hardware_command(
                "main",
                "controller-session",
